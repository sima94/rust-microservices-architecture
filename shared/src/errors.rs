...
impl ResponseError for ServiceError {
    fn error_response(&self) -> HttpResponse {
        match self {
            ...
            ServiceError::Forbidden(msg) => HttpResponse::Forbidden().json(
                serde_json::json!({"error": "forbidden", "error_description": msg}),
            ),
            ServiceError::Conflict(msg) => HttpResponse::Conflict().json(
                serde_json::json!({"error": "conflict", "error_description": msg}),
            ),
            ServiceError::BadRequest(msg) => HttpResponse::BadRequest().json(
                serde_json::json!({"error": "bad_request", "error_description": msg}),
            ),
