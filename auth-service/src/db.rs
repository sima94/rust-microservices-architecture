...
pub async fn init_pools() -> DbPools {
    let write_url = std::env::var("DATABASE_URL").expect("DATABASE_URL must be set");

    let read_url = std::env::var("DATABASE_READ_URL").unwrap_or_else(|_| write_url.clone());
    ...
    println!(
        "DB pools initialized (write: {}, read: {})",
